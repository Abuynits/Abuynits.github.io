...
def generate_compute_timeline_graph(data):
    ...
    ax_graph.plot(
        range(len(df)),
        df["Max TFLOPS Used (Mixed)"],
        linewidth=3,
        color="#FF6B6B",
        alpha=0.6,
        label="Max TFLOPS Used (Mixed)",
        linestyle="--",
    )

    ax_graph.plot(
        range(len(df)),
        df["Max Cumulative TFLOPS Used (Mixed)"],
        linewidth=3,
        color="#4ECDC4",
        alpha=0.8,
        label="Max Cumulative TFLOPS Used (Mixed)",
    )
    ...
    for i, row in df.iterrows():
        ...
        if label_value >= 1.0:
            ...
            ax_graph.text(
                i,
                row["Max Cumulative TFLOPS Used (Mixed)"] * 0.9,
                f"{label_value:,.1f}",
                fontsize=10,
                ha="center",
                va="top",
    ...
    for i, row in df.iterrows():
        if row["Status/Purpose"]:
            ax_graph.scatter(
                i,
                row["Max Cumulative TFLOPS Used (Mixed)"],
                s=150,
    ...
    ax_graph.set_ylabel(
        "TFLOPS (Mixed Precision) (Log Scale)", fontsize=12, fontweight="bold"
    )
    ax_graph.set_title(
        "Happiness ∝ log(TFLOPS)", fontsize=18, fontweight="bold", pad=20
    )
    ...
    for i, row in df.iterrows():
        if row["Status/Purpose"]:
            ...
            color_marker = "●"
            table_data.append(
                [row["Year Acquired"], row["Status/Purpose"], row["Used Configuration"]]
            )
...
COMPUTE_DATA = {
    ...
    "Status/Purpose": [
        "",
        "Started tinkering in ML using Torch / TF tutorials",
        "Gifted by a mentor :)",
        "Colab Pro tier from TE Connectivity AI Cup",
        "Joined CoRAL Lab",
        "starting interning @ Armada AI",
        "Joined CoMMA Lab",
        "left CoRAL, CoMMA, Armada AI. Started interning at Persona AI",
        "setup ssh connections to unused / idle PCs at Persona",
        "setup ssh connections to pcs in other office at persona",
        "Left Persona. No Gpus. Big Sad.",
        "Joined Wang Lab @ UCSD",
        "Access to NRP Compute Cluster. We made it in life.",
    ],
    "Max TFLOPS Used (Mixed)": [
        0.3,
        65.0,
        1.3,
        125.0,
    ...
    "Max Cumulative TFLOPS Used (Mixed)": [
        0.3,
        65.0,
        65.0,
        125.0,
